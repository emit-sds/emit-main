...
class Acquisition:
    ...
    def __init__(self, config_path, acquisition_id):
        ...
        if "daac_scene" in self.metadata:
            ...
            self.min_granule_ur = f"EMIT_L2B_MIN_{self.collection_version}_{daac_start_time_str}_{self.orbit}_{self.daac_scene}"
            self.minuncert_granule_ur = f"EMIT_L2B_MINUNCERT_{self.collection_version}_{daac_start_time_str}_{self.orbit}_{self.daac_scene}"
