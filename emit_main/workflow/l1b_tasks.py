...
class L1BCalibrate(SlurmJobTask):
    ...
    def work(self):
        ...
        hdr["emit data product version"] = wm.config["prod_versions"]["l1b"]
...
class L1BGeolocate(SlurmJobTask):
    ...
    def work(self):
        ...
        for id in acquisition_ids:
            ...
            wm.symlink(acq.glt_img_path, os.path.join(orbit.l1b_dir, f"acquisition_products_v{wm.config['prod_versions']['l1b']}", os.path.basename(acq.glt_img_path)))
            wm.symlink(acq.glt_hdr_path, os.path.join(orbit.l1b_dir, f"acquisition_products_v{wm.config['prod_versions']['l1b']}", os.path.basename(acq.glt_hdr_path)))
            wm.symlink(acq.loc_img_path, os.path.join(orbit.l1b_dir, f"acquisition_products_v{wm.config['prod_versions']['l1b']}", os.path.basename(acq.loc_img_path)))
            wm.symlink(acq.loc_hdr_path, os.path.join(orbit.l1b_dir, f"acquisition_products_v{wm.config['prod_versions']['l1b']}", os.path.basename(acq.loc_hdr_path)))
            wm.symlink(acq.obs_img_path, os.path.join(orbit.l1b_dir, f"acquisition_products_v{wm.config['prod_versions']['l1b']}", os.path.basename(acq.obs_img_path)))
            wm.symlink(acq.obs_hdr_path, os.path.join(orbit.l1b_dir, f"acquisition_products_v{wm.config['prod_versions']['l1b']}", os.path.basename(acq.obs_hdr_path)))
            wm.symlink(acq.rdn_kmz_path, os.path.join(orbit.l1b_dir, f"acquisition_products_v{wm.config['prod_versions']['l1b']}", os.path.basename(acq.rdn_kmz_path)))
            wm.symlink(acq.rdn_png_path, os.path.join(orbit.l1b_dir, f"acquisition_products_v{wm.config['prod_versions']['l1b']}", os.path.basename(acq.rdn_png_path)))
        ...
        ae_nc.title = f"EMIT L1B Corrected Spacecraft Attitude and Ephemeris V0{wm.config['prod_versions']['l1b']}"
...
class L1BRdnFormat(SlurmJobTask):
    ...
    def work(self):
        ...
        cmd = ["python", output_generator_exe, tmp_daac_rdn_nc_path, tmp_daac_obs_nc_path, acq.rdn_img_path, acq.obs_img_path, acq.loc_img_path,
               acq.glt_img_path, "V0" + str(wm.config["prod_versions"]["l1b"]), wm.config["extended_build_num"],
